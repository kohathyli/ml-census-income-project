...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train and evaluate census income Extra Trees classifier.")
    ...
    parser.add_argument("--output-dir", type=str, default="artifacts/extra_trees_classifier")
...
def choose_threshold(
    ...
    for thr in candidate_thresholds:
        ...
        if strategy == "f1":
            ...
            objective = precision if recall >= min_recall else -np.inf
        else:
...
def evaluate_split(
    ...
    save_roc_curve(
        ...
        title=f"Extra Trees ROC Curve ({name.title()} Set)",
    )
...
def compute_feature_importance(pipeline: Pipeline, output_path: Path) -> None:
    ...
    importance_df.head(30).to_csv(output_path, index=False)
...
def main() -> None:
    ...
    # normalize training weights for stability
    ...
    save_json(
        {
            ...
            "model": "Weighted Extra Trees with One-Hot Encoding",
        },
        output_dir / "metrics.json",
    )

    compute_feature_importance(pipeline, output_dir / "feature_importance_top30.csv")
    dump(pipeline, output_dir / "extra_trees_pipeline.joblib")

    print("Extra Trees classification complete.")
