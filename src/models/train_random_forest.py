...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train and evaluate census income random forest classifier.")
    ...
    parser.add_argument("--output-dir", type=str, default="artifacts/random_forest_classifier")
...
def evaluate_split(
    ...
    save_roc_curve(
        ...
        title=f"Random Forest ROC Curve ({name.title()} Set)",
    )
...
def main() -> None:
    ...
    # normalize survey weights for training stability
    ...
    save_json(
        {
            ...
            "model": "Weighted Random Forest with One-Hot Encoding",
        },
        output_dir / "metrics.json",
    )

    compute_feature_importance(pipeline, output_dir / "feature_importance_top30.csv")
    dump(pipeline, output_dir / "random_forest_pipeline.joblib")
